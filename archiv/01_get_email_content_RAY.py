# this is a version to test multiprocessing with ray
...
#
    ...
    def save_to_azure_with_logging(self, azure_table):
        try:
            self.save_to_azure(azure_table)
            success_logger.info(f"Success upload email with id: {self.message_id}")
        except:
            failure_logger.error(f"Could not upload email with id: {self.message_id}")
...
def main():
    ...
    # Create an actor pool with the specified number of actors
    pool = ActorPool([EmailMime.remote(message_id, access_token, user_id) for message_id in message_ids])

    # Process emails in parallel and log the results
    batch_size = 10  # Adjust the batch size as needed
    num_batches = len(message_ids) // batch_size + 1
    results = []
    for i in range(num_batches):
        start_idx = i * batch_size
        end_idx = min((i + 1) * batch_size, len(message_ids))
        batch_results = list(pool.map(lambda actor, _: actor.save_to_azure_with_logging.remote(azure_table), [None] * (end_idx - start_idx)))
        ready_results, _ = ray.wait(batch_results, num_returns=(end_idx - start_idx))
        results.extend(ray.get(ready_results))
