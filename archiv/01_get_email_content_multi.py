# this is a version to test multithreading
#did not work good, got lots of errors in emails
#need to dig deeper into it, but processing time to get emails is not that long
...
def get_folder_id(folder_name):
    ...
    folder_url = f"https://graph.microsoft.com/v1.0/users/{OUTLOOK_API_USER_ID}/mailFolders"
...
def get_subfolder_id(subfolder_name, folder_id):
    
        # Set the URL and headers
        subfolder_url = f"https://graph.microsoft.com/v1.0/users/{OUTLOOK_API_USER_ID}/mailFolders/{folder_id}/childFolders"
        headers = {'Authorization': f"Bearer {token}", 'Content-Type': "application/json"}
...
def get_email_ids_by_daterange(start_time, end_time, folder_id, subfolder_id):
        
        if subfolder_id is not None:
            # Set the URL and headers
            message_url = f"https://graph.microsoft.com/v1.0/users/{OUTLOOK_API_USER_ID}/mailFolders/{folder_id}/childFolders/{subfolder_id}/messages"
        else:
            # Set the URL and headers
            message_url = f"https://graph.microsoft.com/v1.0/users/{OUTLOOK_API_USER_ID}/mailFolders/{folder_id}/messages"
...
class EmailMime:
    def __init__(self, mime, message_id) :
...
async def make_process(message_ids ):
        global START_TIME
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            email_objects = [result for result in results if result is not None]
            save_emails_to_azure(email_objects)


def fetch_mime(session, message_id):
    headers = {'Authorization': f"Bearer {token}"}
    url = f"https://graph.microsoft.com/v1.0//users/{OUTLOOK_API_USER_ID}/messages/{message_id[0]}/$value"
    with session.get(url, headers=headers) as response:
        try:
            email = EmailMime(response.content, message_id)
...
def save_emails_to_azure(email_objects):
    azure_table = AzureTable()
    for email in email_objects:
        ...
        try:
            ...
            success_logger.info(f"Success upload email with id: {email.message_id}")
        except:
            failure_logger.error(f"Could not upload email with id: {email.message_id}")             
...
def main():
    ...
    # Get the folder id
    folder_id = get_folder_id("Posteingang")

    # Get the subfolder id if a subfolder name is not none

    if args.subfolder_name is not None:
        subfolder_id = get_subfolder_id(args.subfolder_name, folder_id)
    else:
        subfolder_id = None
    # Get the message ids
    message_ids = get_email_ids_by_daterange(start_time, end_time, folder_id, subfolder_id)
    print(f"Found {len(message_ids)} emails")
    # Get the email content
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(make_process(message_ids))
